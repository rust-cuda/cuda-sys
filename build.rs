...
fn find_library_paths() -> Vec<String> {
    if let Ok(path) = env::var("CUDA_LIBRARY_PATH") {
        ...
        // If CUDA_LIBRARY_PATH is not found, then CUDA_PATH will be used when building for Windows
        // to locate the Cuda installation. Cuda installs the full Cuda SDK for 64-bit, but only
        // a limited set of libraries for 32-bit. Namely, it does not include cublas in 32-bit,
        // which cuda-sys requires.

        // 'path' points to the base of the CUDA Installation. The lib directory is a sub-directory.
        ...
        // We check that we're building for Windows. This code assumes that the layout in CUDA_PATH
        // matches Windows.
        if target_components[2] != "windows" {
            println!(
                "INFO: The CUDA_PATH variable is only used by cuda-sys on Windows. Your target is \
                 {}.",
                target
            );
            return vec![];
